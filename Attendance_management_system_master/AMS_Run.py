...
def manually_fill():
    ...
    def err_screen_for_subject():
        ...
        Button(ec, cursor='hand2', text='OK', command=ec_delete, fg="black", bg="light gray", width=5, height=1, activebackground="Red",
               font=('times', 15)).place(x=110, y=50)
        ...
        if subb=='':
            ...
            def err_screen1():
                ...
                Label(errsc2, text='Please enter Student & Enrollment!!!', fg='red', bg='white',
                      font=('times', 16)).pack()
                Button(errsc2, cursor='hand2', text='OK', command=del_errsc2, fg="black", bg="light gray", width=5, height=1,
                       activebackground="Red", font=('times', 15)).place(x=110, y=50)
            ...
            ENR = tk.Label(MFW, text="Enter Enrollment", width=15, height=2, fg="black", bg="snow",
                           font=('times', 15, ' bold '))
            ENR.place(x=30, y=100)

            STU_NAME = tk.Label(MFW, text="Enter Student name", width=15, height=2, fg="black", bg="snow",
                                font=('times', 15, ' bold '))
            STU_NAME.place(x=30, y=200)

            global ENR_ENTRY
            ENR_ENTRY = tk.Entry(MFW, width=20,validate='key', bg="white", fg="red", font=('times', 23))
            ...
            STUDENT_ENTRY = tk.Entry(MFW, width=20, bg="white", fg="red", font=('times', 23))
            ...
            def create_csv():
                ...
                csv_name='C:/Users/Tanishq/OneDrive/Desktop/Attendance_management_system_master/Attendance/Manual Attendance/'+DB_table_name+'.csv'
            ...
            Notifi = tk.Label(MFW, text="CSV created Successfully", bg="snow", fg="black", width=33,
                                height=2, font=('times', 19, 'bold'))


            c1ear_enroll = tk.Button(MFW, text="Clear", command=remove_enr, fg="black", bg="light gray", width=10,
                                     height=1,
                                     activebackground="Red", font=('times', 15, ' bold '))
            c1ear_enroll.place(x=690, y=100)

            c1ear_student = tk.Button(MFW, text="Clear", command=remove_student, fg="black", bg="light gray", width=10,
                                      height=1,
                                      activebackground="Red", font=('times', 15, ' bold '))
            ...
            def attf():
                import subprocess
                subprocess.Popen(r'explorer /select,"C:\Users\Tanishq\OneDrive\Desktop\Attendance_management_system_master\Attendance\Manual Attendance\-------Check atttendance-------"')
    ...
    SUB_ENTRY = tk.Entry(sb, width=20, bg="white", fg="red", font=('times', 23))
    SUB_ENTRY.place(x=210, y=105)

    fill_manual_attendance = tk.Button(sb, text="Fill Attendance",command=fill_attendance, fg="black", bg="light green", width=20, height=2,
                       activebackground="Red", font=('times', 15, ' bold '))
...
def subjectchoose():
    def Fillattendances():
        ...
        if time.time() < future:
            if sub == '':
                ...
                while True:
                    ...
                    for (x, y, w, h) in faces:
                        ...
                        if (conf <70):
                            ...
                            date = datetime.datetime.fromtimestamp(ts).strftime('%Y-%m-%d')
                            timeStamp = datetime.datetime.fromtimestamp(ts).strftime('%H:%M:%S')
                            aa = df.loc[df['Enrollment Number'] == Id]['Name'].values
                            global tt
                            tt = str(Id) + "-" + aa
                            En = '15624031' + str(Id)
                    ...
                    attendance = attendance.drop_duplicates(['Enrollment Number'], keep='first')
                ...
                root.configure(background='snow')
                cs = 'C:/Users/Tanishq/OneDrive/Desktop/Attendance_management_system_master/' + fileName
    ...
    def Attf():
        import subprocess
        subprocess.Popen(r'explorer /select,"C:\Users\Tanishq\OneDrive\Desktop\Attendance_management_system_master\Attendance\-------Check atttendance-------"')
    ...
    def log_in():
        ...
        if username == 'Tanishq' :
            if password == 'oriental':
                ...
                cs = 'C:/Users/Tanishq/OneDrive/Desktop/Attendance_management_system_master/StudentDetails/StudentDetails.csv'
                with open(cs, newline="") as file:
                    reader = csv.reader(file)
                    r = 0
...
lbl = tk.Label(window, text="Enter Enrollment", width=20, height=2, fg="black", bg="snow", font=('times', 15, ' bold '))
...
txt = tk.Entry(window, validate="key", width=20, bg="white", fg="red", font=('times', 25))
#txt['validatecommand'] = (txt.register(testVal),'%P','%d')
txt.place(x=550, y=210)

lbl2 = tk.Label(window, text="Enter Name", width=20, fg="black", bg="snow", height=2, font=('times', 15, ' bold '))
lbl2.place(x=200, y=300)

txt2 = tk.Entry(window, width=20, bg="white", fg="red", font=('times', 25))
txt2.place(x=550, y=310)

clearButton = tk.Button(window, cursor="hand2", text="Clear",command=clear,fg="black"  ,bg="light gray"  ,width=10  ,height=1 ,activebackground = "Red" ,font=('times', 15, ' bold '))
clearButton.place(x=950, y=210)

clearButton1 = tk.Button(window, cursor="hand2", text="Clear",command=clear1,fg="black"  ,bg="light gray"  ,width=10 ,height=1, activebackground = "Red" ,font=('times', 15, ' bold '))
clearButton1.place(x=950, y=310)

AP = tk.Button(window, cursor="hand2", text="Check Register students",command=admin_panel,fg="black"  ,bg="cyan"  ,width=19 ,height=1, activebackground = "Red" ,font=('times', 15, ' bold '))
AP.place(x=990, y=410)

takeImg = tk.Button(window, cursor="hand2", text="Take Images",command=take_img,fg="black"  ,bg="light blue"  ,width=20  ,height=3, activebackground = "Red" ,font=('times', 15, ' bold '))
takeImg.place(x=90, y=500)

trainImg = tk.Button(window, cursor="hand2", text="Train Images",fg="black",command=trainimg ,bg="light green"  ,width=20  ,height=3, activebackground = "Red" ,font=('times', 15, ' bold '))
trainImg.place(x=390, y=500)

FA = tk.Button(window, cursor="hand2", text="Automatic Attendace",fg="black",command=subjectchoose  ,bg="light blue"  ,width=20  ,height=3, activebackground = "Red" ,font=('times', 15, ' bold '))
FA.place(x=690, y=500)

quitWindow = tk.Button(window, cursor="hand2", text="Manually Fill Attendance", command=manually_fill  ,fg="black"  ,bg="light green"  ,width=20  ,height=3, activebackground = "Red" ,font=('times', 15, ' bold '))
